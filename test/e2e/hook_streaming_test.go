...
func TestHookOutputStreaming(t *testing.T) {
	...
	// Verify worktree was created (default location is ../worktrees/<repo>/branch-name)
	worktreePath := filepath.Join(repo.Path(), "..", "worktrees", filepath.Base(repo.Path()), "test-branch")
